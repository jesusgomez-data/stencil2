...
export default function TiendaHombrePage() {
  const [searchQuery, setSearchQuery] = useState('')
  const [selectedModel, setSelectedModel] = useState<GlassesModel | 'all'>('all')
  const [showMobileFilters, setShowMobileFilters] = useState(false)

  // Filter products for men or unisex
  const filteredProducts = PRODUCTS.filter((p) => {
    const isMenProduct = p.gender === 'men' || p.gender === 'unisex'
    ...
    return isMenProduct && matchesSearch && matchesModel
  })
      ...
      <main className="flex-1 pt-28 pb-16 px-6 lg:px-10 max-w-[1400px] mx-auto w-full">
        ...
        <div className="border-b border-white/[0.07] pb-8 mb-10 text-left">
          ...
          <p className="font-code text-[11px] text-white/40 max-w-xl mt-3 leading-relaxed">
            Diseñados para destacar. Modelos con líneas marcadas y ajuste óptimo. Lentes polarizadas e ingeniería técnica para el día a día.
          </p>
        </div>
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredProducts.map((p) => (
              <div key={p.id} className="h-96 border border-white/[0.052] rounded-2xl overflow-hidden hover:border-white/15 transition-all">
                <ProductCard
                  code={p.code}
                  letter={p.letter}
                  name={p.name}
